...
export const writePost = ({ title, content, name, pass }) => {
  ...
  return client.post(`/WebProject/community/post?${queryString}`)
}
...
export const writeReply = ({ boardno, name, content, pass }) => {
  ...
  return client.post(`/WebProject/community/post/reply?${queryString}`)
}

// 게시글 상세보기
export const readPost = no =>
  client.get(`/WebProject/community/post/${no}`);
...
export const listPosts = ({ pagenum, type, keyword }) => {
  ...
  return client.get(`/WebProject/community?${queryString}`);
}
...
export const updatePost = ({ no, title, content, pass }) => {
  ...
  return client.post(`/WebProject/community/post/${no}?${queryString}`)
}

// 댓글 업데이트 (댓글 번호, 내용, 비밀번호확인)
export const updateReply = ({ replyNo, contentUpdate, passUpdate }) =>
  client.post(`/WebProject/community/post/reply/${replyNo}?content=${contentUpdate}&pass=${passUpdate}`)
...
export const removePost = ({ no, pass }) => {
  ...
  return client.delete(`/WebProject/community/post/${no}?${queryString}`)
}
...
export const replyDelete = ({ replyNo, pass }) => {
  ...
  return client.delete(`/WebProject/community/post/reply/${replyNo}?${queryString}`)
}